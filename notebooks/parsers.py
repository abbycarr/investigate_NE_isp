            ...
            if isinstance(row, dict):
                ...
                if internet:
                    ...
                    for plan in internet["basePlans"]:
                        ...
                        speed_up = (
                            offer["uploadSpeed"]["speed"]
                            if "FIBER" not in package
                            else speed_down
                        )
                        if speed_unit == "Kbps":
                            speed_down = speed_down * 0.001
                            speed_up = speed_up * 0.001
                            speed_unit = "Mbps"
                        plans.append(
                            {
                                "package": package,
                    ...
                    fastest_plan = plans.sort_values(
                        by="speed_down", ascending=False
                    ).iloc[0]
                    return pd.DataFrame(
                        [
                            dict(
...
def parse_cl(row: dict):
    ...
    record = {
        ...
        "incorporated_place": incorporated_place,
        "major_city": row["major_city"],
        "state": [_["state"] for _ in cities if _["city"].lower() == row["major_city"]][
            0
        ],
        "lat": lat,
        "lon": lon,
...
def get_cheapest_speed_hsi(row: dict):
    ...
    for service in products:
        ...
        if "Internet" in service_name:
            ...
            speed_down = int(speed_down) * 0.000001
            speed_up = int(speed_up) * 0.000001
            internet = dict(
                speed_down=speed_down,
                speed_up=speed_up,
                speed_unit="Mbps",
    ...
    cheapest_offer = (
        pd.DataFrame(plans)
        .sort_values(by=["price", "speed_down"], ascending=[True, False])
        .iloc[0]
    )
    fastest_offer = (
        pd.DataFrame(plans).sort_values(by=["speed_down"], ascending=False).iloc[0]
    )
...
def get_cheapest_speed_fios(row: dict):
    ...
    if plans:
        cheapest_offer = (
            pd.DataFrame(plans).sort_values(by="price", ascending=True).iloc[0]
        )
        fastest_offer = (
            pd.DataFrame(plans).sort_values(by="speed_down", ascending=False).iloc[0]
        )
...
def parse_verizon(row: dict, include_offer_meta=False):
    ...
    incorporated_place = get_incorporated_places(row)
    in_service = (
        row.get("availability_qualifications", {}).get("data", {}).get("inService")
    )
    record = {
        "address_full": row["address_full"],
        "incorporated_place": incorporated_place,
...
def get_cheapest_speed_el(row: dict):
    ...
    if offers:
        ...
        if providers:
            ...
            code2meta = {_.get("level"): _ for _ in providers}
        else:
            code2meta = {}
        ...
        for offer in offers:
            ...
            provider = (
                code2meta.get(serv_level, {}).get("vendor", "").replace(" IMA", "")
            )
            ...
            plans.append(
                dict(
                    ...
                    speed_down=float(
                        name2speed_el.get(service_name.lower(), service_name)
                    ),
