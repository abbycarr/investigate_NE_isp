...
def bucket_and_bin(df, limitations=False):
    ...
    try:
        df["race_quantile"] = aspirational_quartile(
            df.race_perc_non_white, labels=race_labels
        )
    ...
    if limitations:
        ...
        df["income_level"] = pd.cut(
            df["income_lmi"],
            bins=[-1e10, 0.5, 1.2, 1e10],
            labels=["Low", "Middle", "Upper Income"],
        )

    # this is our DV
    df["is_slow"] = df.apply(
        lambda x: 1 if x["speed_down_bins"] == "Slow (<25 Mbps)" else 0, axis=1
    )
...
def speed_breakdown(df, isp, location="National"):
    ...
    plt.title(
        f"{isp} {location} Residential Download Speeds (N={len(df):,})",
        loc="left",
        y=1.075,
        size=15.5,
    )
...
def race(df, isp, location="National"):
    ...
    to_plot = (
        ...
        .speed_down_bins.value_counts(normalize=True)
        .sort_index()
        * 100
    ).unstack()[[c for c in speed_labels.keys() if c in categories]][::-1]
    ax = to_plot.plot(
        kind="barh",
        stacked=True,
        figsize=(8, 5),
    ...
    plt.title(
        f"{isp} {location} Residential Download Speeds by \n"
    ...
    for label, h in zip(bin_counts, heights):
        ax.text(101, h + 0.1, f"N={label:,}", ha="left", va="bottom")
...
def income(df, isp, location="National"):
    ...
    df.loc[:, "income_level"] = (
        df["income_level"]
        .astype(CategoricalDtype(income_labels, ordered=True))
        .copy(deep=True)
    )
    ...
    to_plot = (
        ...
        .sort_index()
        * 100
    ).unstack()[[c for c in speed_labels.keys() if c in categories]]
    ...
    plt.title(
        f"{isp} {location} Residential Download Speeds\n"
    ...
    plt.text(
        0,
        -1.4,
        f"Income-levels defined by the Community Reinvestment Act.\n"
...
def redlining(df, isp, location="National"):
    ...
    to_plot = (
        df.groupby("redlining_grade")
        .speed_down_bins.value_counts(normalize=True)
        .sort_index()
        * 100
    ).unstack()[::-1][[c for c in speed_labels.keys() if c in categories]]
    ...
    plt.text(
        0,
        -1.4,
        "Historic HOLC grades digitized by the Mapping Inequality project.\n"
...
def plot_race(df, isp, location="National", price="$55"):
    ...
    n_intervals = 21
    steps = 1 / n_intervals
    intervals = np.arange(0, 1.0 + steps, steps)
    ...
    plt.yticks(
        np.arange(steps, 1 + steps, ((1 - steps) / 2)),
    ...
    ax.grid(
        which="major", axis="x", linestyle="-", alpha=0.27, zorder=1000, color="white"
    )

    ax.grid(
        which="both", axis="y", linestyle="-", alpha=0.27, zorder=1000, color="white"
    )
    ...
    plt.text(
        0,
        1.11,
        f"All prices quoted at {price}. 2019 5-year ACS data (y-axis) bucketed in 5-percent increments."
